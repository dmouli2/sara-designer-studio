...
export default function EditStaffForm({ staff }: { staff: StaffListItem }) {
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      <div>
        <label className="text-xs text-fg-2 mb-1 block">Username</label>
        <p className="text-sm text-fg py-2">{staff.username}</p>
      </div>
      ...
      <div>
        <label htmlFor="active" className="text-xs text-fg-2 mb-1 block">
          Status
        </label>
